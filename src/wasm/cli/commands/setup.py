...
def _handle_init(args: Namespace) -> int:
    ...
    if summary["nodejs"]["installed"]:
        ...
        logger.key_value(
            "  Package Managers", ", ".join(installed_pms) if installed_pms else "npm only"
        )
    else:
...
def _handle_permissions(args: Namespace) -> int:
    ...
    if nginx_available.exists():
        if os.access(nginx_available, os.W_OK):
            ...
            logger.info("Nginx sites-available requires sudo")
    ...
    if systemd_dir.exists():
        if os.access(systemd_dir, os.W_OK):
            ...
            logger.info("Systemd directory requires sudo")
...
def _handle_doctor(args: Namespace) -> int:
    ...
    if command_exists("node"):
        ...
        logger.info(
            "  Fix: curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash - && sudo apt install -y nodejs"
        )
        issues_found += 1
    ...
    if issues_found == 0 and warnings_found == 0:
        ...
        logger.warning(
            f"⚠ {warnings_found} warning(s) found. System can function but some features may be limited."
        )
    else:
