...
class EnvManager:
    ...
    # Substrings that mark a default value as a placeholder rather than a real
    # value. Matched case-insensitively against the .env.example default. When
    # a secret variable has a placeholder default, we regenerate it instead of
    # propagating an unsafe value into the systemd unit.
        ...
        for var in variables:
            ...
            if var.secret and (not var.default or default_is_placeholder):
                ...
                if default_is_placeholder:
                    self.logger.debug(
                        f"Regenerated secret for {var.name} "
                        f"(placeholder default detected)"
                    )
                continue
