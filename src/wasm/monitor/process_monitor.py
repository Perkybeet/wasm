...
class ProcessMonitor:
    ...
    def _load_config(self) -> MonitorConfig:
        ...
        return MonitorConfig(
            ...
            scan_interval=self.global_config.get("monitor.scan_interval", DEFAULT_SCAN_INTERVAL),
            ai_interval=self.global_config.get("monitor.ai_interval", DEFAULT_AI_INTERVAL),
            cpu_threshold=self.global_config.get("monitor.cpu_threshold", DEFAULT_CPU_THRESHOLD),
            ...
            log_file=Path(self.global_config.get("monitor.log_file", "/var/log/wasm/monitor.log")),
        )
    ...
    def _get_processes_fallback(self) -> list[ProcessInfo]:
        ...
        result = run_command(["ps", "-eo", "pid,user,%cpu,%mem,stat,args", "--no-headers"])
    ...
    def _mitigate_threat(
        ...
        should_terminate = self.config.auto_terminate and is_malicious and not self.config.dry_run
        ...
        if should_terminate:
            ...
            if result.recommended_action == "terminate_tree":
                ...
                if terminated:
                    actions_taken.append(f"TERMINATED process tree ({len(terminated)} processes)")
            else:
    ...
    def scan_once(
        ...
        for result in malicious_results:
            self.logger.info(
                f"Processing MALICIOUS threat: {result.process.name} (PID: {result.process.pid})"
            )
...
"""
    ...
    def enable_service(self) -> bool:
        ...
        """
        try:
            result = run_command_sudo(["systemctl", "enable", "--now", self.SERVICE_NAME])
    ...
    def disable_service(self) -> bool:
        ...
        """
        try:
            result = run_command_sudo(["systemctl", "disable", "--now", self.SERVICE_NAME])
    ...
    def stop_service(self) -> bool:
        ...
        try:
            # Use subprocess directly for reliability
            result = subprocess.run(
                ["systemctl", "stop", self.SERVICE_NAME], capture_output=True, text=True, timeout=30
            )
    ...
    def uninstall_service(self) -> bool:
        ...
        try:
            ...
            run_command_sudo(["systemctl", "disable", "--now", self.SERVICE_NAME])
            ...
            try:
                ...
                for proc in psutil.process_iter(["pid", "cmdline"]):
                    try:
                        ...
                        if (
                            cmdline
                            and "wasm" in " ".join(cmdline)
                            and "monitor" in " ".join(cmdline)
                            and "run" in " ".join(cmdline)
                        ):
