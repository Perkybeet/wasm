...
class MongoDBManager(BaseDatabaseManager):
    ...
    def list_users(self) -> list[UserInfo]:
        ...
        for user_data in user_list:
            if isinstance(user_data, dict):
                ...
                databases = list(
                    set(r.get("db", "") for r in roles if isinstance(r, dict) and r.get("db"))
                )
                privileges = list(set(r.get("role", "") for r in roles if isinstance(r, dict)))
    ...
    def backup(
        ...
        **kwargs,
        ...
        self._run_sudo(["tar", "-czf", str(tar_file), "-C", str(self.BACKUP_DIR), dirname])
        ...
        **kwargs,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if backup_path.suffix == ".gz" and ".tar" in backup_path.name:
                result = self._run_sudo(["tar", "-xzf", str(backup_path), "-C", temp_dir])
