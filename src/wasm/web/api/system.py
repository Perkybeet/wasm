...
def _get_system_info() -> SystemInfo:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="psutil not installed. Install with: pip install wasm-cli[web]"
        )
...
async def get_system_info(request: Request, session: dict = Depends(get_current_session)):
    """
    Get comprehensive system information.
    """
...
async def get_cpu_info(request: Request, session: dict = Depends(get_current_session)):
    """
...
async def get_memory_info(request: Request, session: dict = Depends(get_current_session)):
    """
...
async def get_disk_info(request: Request, session: dict = Depends(get_current_session)):
    """
...
async def kill_process(
    ...
    if signal not in [9, 15]:
        raise HTTPException(
            status_code=400, detail="Only signals 9 (KILL) and 15 (TERM) are allowed"
        )
...
@router.get("/network")
async def get_network_info(request: Request, session: dict = Depends(get_current_session)):
    """
...
async def check_version(request: Request, session: dict = Depends(get_current_session)):
    """
    ...
    if not UpdateChecker._is_cache_valid():
        try:
            ...
            if latest_version:
                ...
                cached_data = {"latest_version": latest_version, "has_update": has_update}
        except Exception:
            pass
