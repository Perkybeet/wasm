...
@router.post("/logout")
async def logout(request: Request, session: dict = Depends(get_current_session)):
    """
...
@router.get("/verify")
async def verify_token(request: Request, session: dict = Depends(get_current_session)):
    """
    Verify the current session token is valid.
    """
    return TokenInfo(valid=True, expires_at=session.get("exp"), session_id=session.get("sid"))


@router.get("/sessions")
async def get_sessions(request: Request, session: dict = Depends(get_current_session)):
    """
...
async def revoke_all_sessions(request: Request, session: dict = Depends(get_current_session)):
    """
