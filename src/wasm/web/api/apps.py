...
async def list_apps(request: Request, session: dict = Depends(get_current_session)):
    """
...
async def get_app(domain: str, request: Request, session: dict = Depends(get_current_session)):
    """
...
async def create_app(
    body: CreateAppRequest, request: Request, session: dict = Depends(get_current_session)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to deploy application: {e!s}")


@router.post("/{domain}/restart")
async def restart_app(domain: str, request: Request, session: dict = Depends(get_current_session)):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to restart application: {e!s}")


@router.post("/{domain}/stop")
async def stop_app(domain: str, request: Request, session: dict = Depends(get_current_session)):
    """
    Stop an application.
    """
    from wasm.managers.service_manager import ServiceManager
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to stop application: {e!s}")


@router.post("/{domain}/start")
async def start_app(domain: str, request: Request, session: dict = Depends(get_current_session)):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start application: {e!s}")
...
async def get_app_logs(
    ...
    return AppLogsResponse(domain=validated_domain, logs=logs, lines=lines)
...
async def delete_app(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to delete application: {e!s}")
