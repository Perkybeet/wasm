...
async function pageAt(id: number | string, extra: Record<string, RouteHandler> = {}) {
...
describe("a deployment's page", { timeout: 20_000 }, () => {
  ...
  it("follows a running deploy: its job over the WebSocket, its log as it grows, until it ends", async () => {
    ...
    const running = { ...FAILED, id: 31, status: "running", finished_at: null, duration_s: null, error: null };
    let status = "running";
    ...
    const { queryClient } = await pageAt(31, {
      ...
      "GET /api/jobs/active": () => json(200, { jobs: status === "running" ? [job] : [], total: 1, active: 1 }),
  ...
  it("redeploys and opens the new deploy once it is recorded", async () => {
    let newest = 20;
    const { user, location } = await pageAt(20, {
      "GET /api/deployments": () => json(200, { items: [{ ...FAILED, id: newest }], total: 1, next_before_id: null }),
      ...
      "GET /api/deployments/21": () => json(200, { ...FAILED, id: 21, status: "running", error: null, finished_at: null }),
