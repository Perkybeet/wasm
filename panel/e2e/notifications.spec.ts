...
test("tests each channel and reports what the receiving server answered", async ({ page, consoleServer, problems }) => {
  ...
  try {
    ...
    // A channel with no destination: the notifier says which setting is missing.
    const slack = page.getByRole("article", { name: "Slack" });
    await slack.getByRole("button", { name: "Send test" }).click();
    await expect(slack.getByText("Channel slack is not configured; set notifications.channels.slack.webhook_url first.")).toBeVisible();
    ...
    await webhookAfter.getByRole("button", { name: "Send test" }).click();
    await expect(webhookAfter.getByText(/Channel webhook is not configured/)).toBeVisible();
  } finally {
