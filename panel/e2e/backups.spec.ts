...
function toasts(page: import("@playwright/test").Page) {
...
test("every seeded backup is listed, and the page passes axe", async ({ page, consoleServer }) => {
  ...
  // seed_console_state gives 4 apps two backups each (8); seed_app_tabs' rollback demo adds two
  // more for pedidos.cittek.es (10).
...
test("restoring a backup is confirmed by typing the target domain", async ({ page, consoleServer }) => {
  await signIn(page, consoleServer, "/backups");
