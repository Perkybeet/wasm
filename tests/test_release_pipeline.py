...
def test_a_redeploy_as_a_static_type_retires_the_unit_of_the_process_type(
    ...
    The unit the process type wrote is deleted through the service manager
    (which enforces ownership), as the last step, once the site serves files.
    """
    machine.git.publish(node_tree(tmp_path / "v1"))
    deploy_new(root, machine)
    assert "rel-example-com" in machine.services.units

    machine.git.publish(write_tree(tmp_path / "site", {"public/index.html": "<h1>hi</h1>"}))
    output: list[str] = []
    deployer = wire(StaticDeployer(verbose=False, runner=machine.runner), machine)
    deployer.logger.step = lambda i, n, title, icon="": output.append(title)
    deployer.configure(DOMAIN, GIT_URL, port=PORT, ssl=False, app_path=root, layout=RELEASES)
    assert deployer.deploy() is True

    assert machine.services.units == {}
    assert output[-1] == "Removing the unit a previous deployment left"
    assert machine.web.sites[DOMAIN]["template"] == "static"
...
def test_a_redeploy_of_a_process_type_keeps_its_unit(
    ...
    assert "rel-example-com" in machine.services.units
    assert "Removing the unit a previous deployment left" not in output
...
def test_an_update_of_a_static_app_retires_a_leftover_unit(
    ...
    steps: list[str] = []
    outcome = update(machine, monkeypatch, deployer_class=StaticDeployer, on_step=steps.append)

    assert machine.services.units == {}
    assert "Removing the unit a previous deployment left" in steps
...
def test_a_unit_the_service_manager_refuses_to_delete_does_not_fail_the_deploy(
    ...
    machine.git.publish(write_tree(tmp_path / "site", {"public/index.html": "<h1>hi</h1>"}))
    machine.services.units["rel-example-com"] = {"command": "/usr/bin/something-else"}

    def refuse(name: str) -> None:
        raise ServiceError(
            f"Refusing to delete a unit WASM does not manage: {name}",
            details="/etc/systemd/system/rel-example-com.service was not generated by WASM",
        )

    machine.services.delete_service = refuse
    warnings: list[str] = []
    deployer = wire(StaticDeployer(verbose=False, runner=machine.runner), machine)
    deployer.logger.warning = warnings.append
    deployer.configure(DOMAIN, GIT_URL, port=PORT, ssl=False, app_path=root, layout=RELEASES)

    assert deployer.deploy() is True
    assert "rel-example-com" in machine.services.units
    assert any("was left in place" in w and "does not manage" in w for w in warnings)
